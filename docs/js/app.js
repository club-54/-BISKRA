...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Load menu + supplements concurrently (static: via API layer → JSONBin + static files)
  try {
    [state.menu, state.supplements] = await Promise.all([
      API.getMenu(),
      API.getSupplements()
    ]);
  } catch {
...
function bindPromoEvents() {
  ...
  newBtn.addEventListener('click', async () => {
    ...
    try {
      const data = await API.validatePromo(code);
...
async function submitOrder(form, total) {
  ...
  try {
    const json = await API.submitOrder({
      name, phone, address, gps, note, total,
      cart:  state.cart.map(i => ({ name: i.name, size: i.size || null, qty: i.qty, price: i.price })),
      promo: state.promo ? { code: state.promo.code, discount: state.promo.discount, savedAmount } : null,
    });
...
async function renderJuiceSection() {
  ...
  try {
    const data = await API.getJuices();
...
async function loadGallery() {
  try {
    ...
    grid.innerHTML = items.map((item, i) => `
      <div class="gallery-item${i === 0 ? ' gallery-item--big' : ''}" data-index="${i}">
    ...
    if (typeof window.bindGalleryLightbox === 'function') window.bindGalleryLightbox();
  } catch (e) {
